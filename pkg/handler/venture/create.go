...
func (h *Handler) Create(ctx context.Context, req *venture.CreateI) (*venture.CreateO, error) {
...

	{
		...
		for i := range req.Obj {
...

			{
				...
				req.Obj[i].Metadata[metadata.VentureID] = strconv.FormatInt(time.Now().UTC().UnixNano(), 10)
			}
		}
	}
	...
	{
		ok, err := h.storage.Venture.Creator.Verify(ctx, req)
		if err != nil {
			return nil, tracer.Mask(err)
		}
		...
		res, err := h.storage.Venture.Creator.Create(req)
		if err != nil {
			return nil, tracer.Mask(err)
		}
